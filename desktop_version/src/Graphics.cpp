...
#define PROCESS_TILESHEET_CHECK_ERROR(tilesheet, tile_square)                  \
    if (grphx.im_##tilesheet->w % tile_square != 0 ||                          \
        ...
        const char* error =                                                    \
            "Error: %s.png dimensions not exact multiples of %i!";             \
        char message[128];                                                     \
        SDL_snprintf(                                                          \
            message, sizeof(message), error, #tilesheet, tile_square);         \
                                                                               \
        const char* error_title = "Error with %s.png";                         \
        char message_title[128];                                               \
        SDL_snprintf(                                                          \
            message_title, sizeof(message_title), error_title, #tilesheet);    \
                                                                               \
        puts(message);                                                         \
                                                                               \
        SDL_ShowSimpleMessageBox(                                              \
            SDL_MESSAGEBOX_ERROR, message_title, message, NULL);               \
                                                                               \
        exit(1);                                                               \
    }
...
#define PROCESS_TILESHEET_RENAME(tilesheet, vector, tile_square, extra_code)   \
    ...
    for (int j = 0; j < grphx.im_##tilesheet->h / tile_square; j++) {          \
        for (int i = 0; i < grphx.im_##tilesheet->w / tile_square; i++) {      \
            SDL_Surface* temp = GetSubSurface(grphx.im_##tilesheet,            \
                                              i * tile_square,                 \
                                              j * tile_square,                 \
                                              tile_square,                     \
                                              tile_square);                    \
...
void Graphics::map_option(int opt,
                          int num_opts,
                          const std::string& text,
                          bool selected /*= false*/)
{
    ...
    int yoff = -(num_opts * 12) / 2; // could be simplified to -num_opts * 6,
                                     // this conveys my intent better though
    yoff += opt * 12;
...
void Graphics::PrintAlpha(int _x,
                          int _y,
                          std::string _s,
                          int r,
                          int g,
                        ...
                        int sc)
{
    ...
    while (iter != _s.end()) {
        ...
        */
        ...
        idx = font_idx(curr);
        if (INBOUNDS(idx, font)) {
            SDL_Surface* tempPrint =
                ScaleSurface(font[idx], font[idx]->w * sc, font[idx]->h * sc);
            SDL_Rect printrect = { static_cast<Sint16>((_x) + bfontpos),
                                   static_cast<Sint16>(_y),
                                   static_cast<Sint16>((bfont_rect.w * sc) + 1),
                                   static_cast<Sint16>((bfont_rect.h * sc) +
                                                       1) };
...
void Graphics::PrintOffAlpha(int _x,
                             int _y,
                             std::string _s,
                             int r,
                             int g,
...
void Graphics::bprintalpha(int x,
                           int y,
                           std::string t,
                           int r,
                           int g,
...
void Graphics::drawtile(int x, int y, int t)
{
    ...
    if (t >= 14 && t <= 17 &&
        (!FILESYSTEM_assetsmounted || ed.onewaycol_override)) {
...
void Graphics::drawgui()
{
    ...
    for (size_t i = 0; i < textbox.size(); i++) {
        ...
        textbox[i].setcol(textbox[i].tr * tl_lerp,
                          textbox[i].tg * tl_lerp,
                          textbox[i].tb * tl_lerp);

        if (textbox[i].r == 0 && textbox[i].g == 0 && textbox[i].b == 0) {
            if (flipmode) {
                for (size_t j = 0; j < textbox[i].line.size(); j++) {
                    Print(textbox[i].xp + 8,
                          textbox[i].yp + (textbox[i].line.size() * 8) -
                              (j * 8),
                          textbox[i].line[j],
                          196,
                          196,
            ...
            drawcoloredtile(textbox[i].xp,
                            textbox[i].yp,
                            40,
                            textbox[i].r,
                            textbox[i].g,
                            textbox[i].b);
            drawcoloredtile(textbox[i].xp + textbox[i].w - 8,
                            textbox[i].yp,
                            42,
                            textbox[i].r,
                            textbox[i].g,
            ...
            for (int k = 0; k < textbox[i].lw; k++) {
                drawcoloredtile(textbox[i].xp + 8 + (k * 8),
                                textbox[i].yp,
                                41,
                                textbox[i].r,
                                textbox[i].g,
        ...
        if ((textbox[i].yp == 12 || textbox[i].yp == 180) &&
            textbox[i].r == 165 && drawspecial) {
            if (flipmode) {
                drawimage(5, 0, 180, true);
            } else {
                drawimage(0, 0, 12, true);
            }
        } else if ((textbox[i].yp == 12 || textbox[i].yp == 180) &&
                   textbox[i].g == 165 && drawspecial) {
            if (flipmode) {
        ...
        if (flipmode) {
            if (textbox[i].r == 175 && textbox[i].g == 175 && drawspecial) {
                ...
                drawsprite(80 - 6,
                           64 + 48 + 4,
                           6,
                           220 - help.glow / 4 - int(fRandom() * 20),
                           120 - help.glow / 4,
                           210 - help.glow / 4);
                ...
                drawsprite(80 - 6,
                           64 + 48 + 4,
                           6,
                           120 - help.glow / 4 - int(fRandom() * 20),
                           220 - help.glow / 4,
                           120 - help.glow / 4);
...
void Graphics::drawimagecol(int t,
                            int xp,
                            int yp,
                            int r = 0,
                            int g = 0,
...
void Graphics::drawcrewman(int x,
                           int y,
                           int t,
                           bool act,
                           bool noshift /*=false*/)
...
void Graphics::drawpixeltextbox(int x,
                                int y,
                                int w,
                                int h,
                                int w2,
...
void Graphics::drawcustompixeltextbox(int x,
                                      int y,
                                      int w,
                                      int h,
                                      int w2,
...
void Graphics::createtextbox(std::string t,
                             int xp,
                             int yp,
                             int r /*= 255*/,
                             int g /*= 255*/,
...
void Graphics::drawfade()
{
    ...
    if ((fademode == 1) || (fademode == 4)) {
        ...
        for (size_t i = 0; i < SDL_arraysize(fadebars); i++) {
            FillRect(
                backBuffer, fadebars[i], i * 16, usethisamount, 16, 0x000000);
        }
    } else if (fademode == 5) {
        for (size_t i = 0; i < SDL_arraysize(fadebars); i++) {
            FillRect(backBuffer,
                     fadebars[i] - usethisamount,
                     i * 16,
                     500,
                     16,
                     0x000000);
...
bool Graphics::Hitest(SDL_Surface* surface1,
                      point p1,
    ...
    if (obj.entities[t].life == 0) {
        switch (linestate) {
            case 0:
                FillRect(backBuffer,
                         line_rect,
                         getRGB(200 - 20, 200 - 20, 200 - 20));
                break;
            case 1:
                FillRect(backBuffer,
                         line_rect,
                         getRGB(225 - 30, 245 - 30, 245 - 30));
                break;
            case 2:
                FillRect(backBuffer,
                         line_rect,
                         getRGB(245 - 30, 245 - 30, 225 - 30));
                break;
            case 3:
                FillRect(backBuffer,
                         line_rect,
                         getRGB(164 - 10, 200 - 20, 200 - 20));
                break;
            case 4:
                FillRect(backBuffer,
                         line_rect,
                         getRGB(224 - 20, 255 - 30, 196 - 20));
                break;
            case 5:
                FillRect(backBuffer,
                         line_rect,
                         getRGB(205 - 20, 235 - 30, 196 - 20));
                break;
            case 6:
                FillRect(backBuffer,
                         line_rect,
                         getRGB(164 - 10, 164 - 10, 164 - 10));
                break;
            case 7:
                FillRect(backBuffer,
                         line_rect,
                         getRGB(225 - 30, 245 - 30, 205 - 20));
                break;
            case 8:
                FillRect(backBuffer,
                         line_rect,
                         getRGB(205 - 20, 255 - 30, 225 - 30));
                break;
            case 9:
                FillRect(backBuffer,
                         line_rect,
                         getRGB(245 - 30, 245 - 30, 245 - 30));
                break;
        }
    } else {
...
void Graphics::drawtrophytext()
{
    ...
    switch (obj.trophytype) {
        ...
        case 8:
            ...
            Print(-1,
                  16,
                  "Complete the game in flip mode",
                  temp,
                  temp2,
                  temp3,
                  true);
    ...
    for (int i = obj.entities.size() - 1; i >= 0; i--) {
        ...
        switch (obj.entities[i].size) {
            case 0: {
                ...
                drawRect.x += tpoint.x;
                drawRect.y += tpoint.y;
                BlitSurfaceColoured((*spritesvec)[obj.entities[i].drawframe],
                                    NULL,
                                    backBuffer,
                                    &drawRect,
                                    ct);
                ...
                bool isInWrappingAreaOfTower =
                    map.towermode && !map.minitowermode && map.ypos >= 500 &&
                    map.ypos <= 5000;
                if (wrapX && (map.warpx || isInWrappingAreaOfTower)) {
                    ...
                    drawRect.y += tpoint.y;
                    BlitSurfaceColoured(
                        (*spritesvec)[obj.entities[i].drawframe],
                        NULL,
                        backBuffer,
                        &drawRect,
                        ct);
            ...
            case 1:
                ...
                drawRect.x += tpoint.x;
                drawRect.y += tpoint.y;
                BlitSurfaceStandard(tiles[obj.entities[i].drawframe],
                                    NULL,
                                    backBuffer,
                                    &drawRect);
                break;
            ...
            case 8: {
                ...
                for (int ii = 0; ii < thiswidth; ii++) {
                    ...
                    if (custom_gray) {
                        ...
                        BlitSurfaceTinted(
                            (*tilesvec)[obj.entities[i].drawframe],
                            NULL,
                            backBuffer,
                            &drawRect,
                            temp_ct);
            ...
            case 5: // Horizontal Line
            {
                ...
                if ((game.swngame == 3 || kludgeswnlinewidth) &&
            ...
            case 7: // Teleporter
                drawtele(xp,
                         yp - yoff,
                         obj.entities[i].drawframe,
                         obj.entities[i].realcol);
                break;
            ...
            case 9: // Really Big Sprite! (2x2)
                ...
                drawRect.y += tpoint.y;
                BlitSurfaceColoured(
                    (*spritesvec)[obj.entities[i].drawframe + 1],
                    NULL,
                    backBuffer,
                    &drawRect,
                    ct);
                ...
                drawRect.y += tpoint.y;
                BlitSurfaceColoured(
                    (*spritesvec)[obj.entities[i].drawframe + 12],
                    NULL,
                    backBuffer,
                    &drawRect,
                    ct);
                ...
                drawRect.y += tpoint.y;
                BlitSurfaceColoured(
                    (*spritesvec)[obj.entities[i].drawframe + 13],
                    NULL,
                    backBuffer,
                    &drawRect,
                    ct);
            ...
            case 12: // Regular sprites that don't wrap
                ...
                if (obj.entities[i].xp < -20 && obj.entities[i].vx > 0) {
                    ...
                    drawRect.y += tpoint.y;
                    BlitSurfaceColoured(
                        tiles[1167], NULL, backBuffer, &drawRect, ct);
                    ...
                    drawRect.y += tpoint.y;
                    BlitSurfaceColoured(
                        tiles[1166], NULL, backBuffer, &drawRect, ct);
                }
                break;
            case 13: {
                ...
                tpoint.y = yp - yoff;
                setcolreal(obj.entities[i].realcol);
                SDL_Rect drawRect = { Sint16(obj.entities[i].xp),
                                      Sint16(obj.entities[i].yp - yoff),
                                      Sint16(sprites_rect.x * 6),
                                      Sint16(sprites_rect.y * 6) };
                SDL_Surface* TempSurface =
                    ScaleSurface((*spritesvec)[obj.entities[i].drawframe],
                                 6 * sprites_rect.w,
                                 6 * sprites_rect.h);
                BlitSurfaceColoured(
                    TempSurface, NULL, backBuffer, &drawRect, ct);
        ...
        case 2:
            ...
            switch (rcol) {
                ...
                case 6:
                    ...
                    switch (spcol) {
                        case 0:
                            bcol2 = RGBflip(
                                0, 16 * backboxint[0], 16 * backboxint[0]);
                            break; // Cyan
                        case 1:
                            bcol2 = RGBflip(0,
                                            (spcoldel + 1) * backboxint[0],
                                            16 * backboxint[0]);
                            break; // Cyan
                        case 2:
                            bcol2 = RGBflip(0, 0, 16 * backboxint[0]);
                            break; // Blue
                        case 3:
                            bcol2 = RGBflip((16 - spcoldel) * backboxint[0],
                                            0,
                                            16 * backboxint[0]);
                            break; // Blue
                        case 4:
                            bcol2 = RGBflip(
                                16 * backboxint[0], 0, 16 * backboxint[0]);
                            break; // Purple
                        case 5:
                            bcol2 = RGBflip(16 * backboxint[0],
                                            0,
                                            (spcoldel + 1) * backboxint[0]);
                            break; // Purple
                        case 6:
                        ...
                        case 7:
                            bcol2 = RGBflip(16 * backboxint[0],
                                            (16 - spcoldel) * backboxint[0],
                                            0);
                            break; // Red
                        case 8:
                            bcol2 = RGBflip(
                                16 * backboxint[0], 16 * backboxint[0], 0);
                            break; // Yellow
                        case 9:
                            bcol2 = RGBflip((spcoldel + 1) * backboxint[0],
                                            16 * backboxint[0],
                                            0);
                            break; // Yellow
                        case 10:
            ...
            for (int i = 0; i < numbackboxes; i++) {
                switch (rcol) {
                    ...
                    case 0:
                        bcol = RGBflip(
                            16, 128 * backboxint[0], 128 * backboxint[0]);
                        break; // Cyan
                    case 1:
                    ...
                    case 2:
                        bcol = RGBflip(
                            128 * backboxint[0], 16, 128 * backboxint[0]);
                        break; // Purple
                    case 3:
                    ...
                    case 4:
                        bcol = RGBflip(
                            128 * backboxint[0], 128 * backboxint[0], 16);
                        break; // Yellow
                    case 5:
                    ...
                    case 6:
                        ...
                        switch (spcol) {
                            ...
                            case 2:
                                ...
                                break; // Blue
                            case 3:
                                bcol = RGBflip((128 - (spcoldel * 8)) *
                                                   backboxint[0],
                                               16,
                                               128 * backboxint[0]);
                                break; // Blue
                            case 4:
                                bcol = RGBflip(128 * backboxint[0],
                                               16,
                                               128 * backboxint[0]);
                                break; // Purple
                            case 5:
                ...
                backboxrect.x =
                    lerp(backboxes[i].x - backboxvx[i], backboxes[i].x);
                backboxrect.y =
                    lerp(backboxes[i].y - backboxvy[i], backboxes[i].y);
    ...
    switch (t) {
        ...
        case 3: // Warp zone (horizontal)
        {
            ...
            if (backgrounddrawn) {
                ...
                for (int j = 0; j < 15; j++) {
                    for (int i = 0; i < 2; i++) {
                        drawtowertile(317 - backoffset + (i * 16),
                                      (j * 16),
                                      temp + 40); // 20*16 = 320
                        drawtowertile(317 - backoffset + (i * 16) + 8,
                                      (j * 16),
                                      temp + 41);
                        drawtowertile(317 - backoffset + (i * 16),
                                      (j * 16) + 8,
                ...
                for (int j = 0; j < 15; j++) {
                    for (int i = 0; i < 21; i++) {
                        drawtowertile(
                            (i * 16) - backoffset - 3, (j * 16), temp + 40);
                        drawtowertile(
                            (i * 16) - backoffset + 8 - 3, (j * 16), temp + 41);
                        drawtowertile(
                            (i * 16) - backoffset - 3, (j * 16) + 8, temp + 80);
                        drawtowertile((i * 16) - backoffset + 8 - 3,
                                      (j * 16) + 8,
                                      temp + 81);
                    }
                }
        ...
        case 4: // Warp zone (vertical)
        {
            ...
            if (backgrounddrawn) {
                ...
                for (int j = 0; j < 16; j++) {
                    for (int i = 0; i < 21; i++) {
                        drawtowertile(
                            (i * 16), (j * 16) - backoffset - 3, temp + 40);
                        drawtowertile(
                            (i * 16) + 8, (j * 16) - backoffset - 3, temp + 41);
                        drawtowertile(
                            (i * 16), (j * 16) - backoffset + 8 - 3, temp + 80);
                        drawtowertile((i * 16) + 8,
                                      (j * 16) - backoffset + 8 - 3,
                                      temp + 81);
                    }
            ...
            for (int j = 0; j < 30; j++) {
                for (int i = 0; i < 40; i++) {
                    if (map.contents[i + map.vmult[j]] > 0)
                        drawforetile(
                            i * 8, j * 8, map.contents[i + map.vmult[j]]);
                }
            }
        } else if (map.tileset == 1) {
            for (int jt = 0; jt < 30; jt++) {
                for (int it = 0; it < 40; it++) {
                    if (map.contents[it + map.vmult[jt]] > 0)
                        drawforetile2(
                            it * 8, jt * 8, map.contents[it + map.vmult[jt]]);
                }
            }
...
void Graphics::drawtowermap()
{
    ...
    for (int j = 0; j < 31; j++) {
        for (int i = 0; i < 40; i++) {
            ...
            if (temp > 0)
                drawtile3(i * 8, (j * 8) - (yoff % 8), temp, map.colstate);
        }
    }
}
    ...
    if (map.tdrawback) {
        ...
        for (int j = 0; j < 32; j++) {
            for (int i = 0; i < 40; i++) {
                ...
                drawtowertile3(
                    i * 8, (j * 8) - (map.bypos % 8) - off, temp, map.colstate);
            }
        }
        ...
        if (map.scrolldir == 0) {
            ...
            for (int i = 0; i < 40; i++) {
                ...
                drawtowertile3(i * 8,
                               29 * 8 - (map.bypos % 8) - map.bscroll,
                               temp,
                               map.colstate);
                temp = map.tower.backat(i, 30, map.bypos);
                drawtowertile3(i * 8,
                               30 * 8 - (map.bypos % 8) - map.bscroll,
                               temp,
                               map.colstate);
                temp = map.tower.backat(i, 31, map.bypos);
                drawtowertile3(i * 8,
                               31 * 8 - (map.bypos % 8) - map.bscroll,
                               temp,
                               map.colstate);
                temp = map.tower.backat(i, 32, map.bypos);
                drawtowertile3(i * 8,
                               32 * 8 - (map.bypos % 8) - map.bscroll,
                               temp,
                               map.colstate);
            }
        }
...
void Graphics::setcol(int t)
{
    ...
    switch (t) {
            // Player Normal
        case 0:
            ct.colour = getRGB(160 - help.glow / 2 - (fRandom() * 20),
                               200 - help.glow / 2,
                               220 - help.glow);
            break;
            // Player Hurt
        ...
        case 5: // Active savepoint
            ct.colour = getRGB(164 + (fRandom() * 64),
                               164 + (fRandom() * 64),
                               255 - (fRandom() * 64));
            break;
        case 6: // Enemy : Red
            ct.colour = getRGB(
                250 - help.glow / 2, 60 - help.glow / 2, 60 - help.glow / 2);
            break;
        case 7: // Enemy : Green
            ct.colour = getRGB(100 - help.glow / 2 - (fRandom() * 30),
                               250 - help.glow / 2,
                               100 - help.glow / 2 - (fRandom() * 30));
            break;
        case 8: // Enemy : Purple
            ct.colour = getRGB(250 - help.glow / 2,
                               20,
                               128 - help.glow / 2 + (fRandom() * 30));
            break;
        case 9: // Enemy : Yellow
            ct.colour = getRGB(250 - help.glow / 2, 250 - help.glow / 2, 20);
            break;
        case 10: // Warp point (white)
            ct.colour = getRGB(255 - (fRandom() * 64),
                               255 - (fRandom() * 64),
                               255 - (fRandom() * 64));
            break;
        case 11: // Enemy : Cyan
        ...
        case 12: // Enemy : Blue
            ct.colour = getRGB(
                90 - help.glow / 2, 90 - help.glow / 2, 250 - help.glow / 2);
            break;
            // Crew Members
            // green
        case 13:
            ct.colour = getRGB(120 - help.glow / 4 - (fRandom() * 20),
                               220 - help.glow / 4,
                               120 - help.glow / 4);
            break;
            // Yellow
        case 14:
            ct.colour = getRGB(220 - help.glow / 4 - (fRandom() * 20),
                               210 - help.glow / 4,
                               120 - help.glow / 4);
            break;
            // pink
        case 15:
            ct.colour = getRGB(
                255 - help.glow / 8, 70 - help.glow / 4, 70 - help.glow / 4);
            break;
            // Blue
        case 16:
        ...
        case 18: // Enemy : Gray
            ct.colour = getRGB(
                130 - help.glow / 2, 130 - help.glow / 2, 130 - help.glow / 2);
            break;
        case 19: // Enemy : Dark gray
            ct.colour = getRGB(
                60 - help.glow / 8, 60 - help.glow / 8, 60 - help.glow / 8);
            break;
            // Purple
        case 20:
            ct.colour = getRGB(220 - help.glow / 4 - (fRandom() * 20),
        ...
        case 21: // Enemy : Light Gray
            ct.colour = getRGB(
                180 - help.glow / 2, 180 - help.glow / 2, 180 - help.glow / 2);
            break;
        case 22: // Enemy : Indicator Gray
            ct.colour = getRGB(
                230 - help.glow / 2, 230 - help.glow / 2, 230 - help.glow / 2);
            break;
        case 23: // Enemy : Indicator Gray
            ct.colour = getRGB(255 - help.glow / 2 - (fRandom() * 40),
                               255 - help.glow / 2 - (fRandom() * 40),
                               255 - help.glow / 2 - (fRandom() * 40));
            break;
        ...
        case 40: // Teleporter in action!
            temp = fRandom() * 150;
            if (temp < 33) {
                ct.colour = RGBf(255 - (fRandom() * 64),
                                 64 + (fRandom() * 64),
                                 64 + (fRandom() * 64));
            } else if (temp < 66) {
                ct.colour = RGBf(64 + (fRandom() * 64),
                                 255 - (fRandom() * 64),
                                 64 + (fRandom() * 64));
            } else if (temp < 100) {
                ct.colour = RGBf(64 + (fRandom() * 64),
        ...
        case 102: // Teleporter in action!
            ...
            if (temp < 33) {
                ...
                ct.colour = getRGB(64 + (fRandom() * 64),
                                   255 - (fRandom() * 64),
                                   64 + (fRandom() * 64));
            } else if (temp < 100) {
                ct.colour = getRGB(64 + (fRandom() * 64),
...
void Graphics::huetilesetcol(int t)
{
    switch (t) {
        case 0:
            setcolreal(getRGB(
                250 - int(fRandom() * 32), 250 - int(fRandom() * 32), 10));
            break;
        case 1:
            setcolreal(getRGB(
                250 - int(fRandom() * 32), 250 - int(fRandom() * 32), 10));
...
Uint32 Graphics::bigchunkygetcol(int t)
{
    ...
    switch (t) {
        ...
        case 2:
            return getRGB(int(160 - help.glow / 2 - (fRandom() * 20)),
                          200 - help.glow / 2,
                          220 - help.glow);
    }
    return 0x00000000;
                         ...
                         float sc)
{
    ...
    while (iter != t.end()) {
        ...
        if (INBOUNDS(idx, font)) {
            SDL_Surface* tempPrint =
                ScaleSurface(font[idx], font[idx]->w * sc, font[idx]->h * sc);
            SDL_Rect printrect = { Sint16((x) + bfontpos),
                                   Sint16(y),
                                   Sint16(bfont_rect.w * sc),
                                   Sint16(bfont_rect.h * sc) };
            BlitSurfaceColoured(tempPrint, NULL, backBuffer, &printrect, ct);
            SDL_FreeSurface(tempPrint);
        }
