...
#define FOREACH_TRACK(track_name)                                              \
    musicWriteBlob.AddFileToBinaryBlob(track_name);
    TRACK_NAMES
#undef FOREACH_TRACK
...
#define FOREACH_TRACK(track_name)                                              \
    ...
    if (index >= 0 && index < musicReadBlob.max_headers) {                     \
        rw = SDL_RWFromMem(musicReadBlob.getAddress(index),                    \
                           musicReadBlob.getSize(index));                      \
        if (rw == NULL) {                                                      \
    ...
    if (currentsong != t) {
        if (t != -1) {
            ...
            if (currentsong == 0 || currentsong == 7 ||
                (!map.custommode && (currentsong == 0 + num_pppppp_tracks ||
                                     currentsong == 7 + num_pppppp_tracks))) {
                // Level Complete theme, no fade in or repeat
                if (Mix_FadeInMusicPos(
                        musicTracks[t].m_music, 0, 0, position_sec) == -1) {
                ...
                if (Mix_FadingMusic() == MIX_FADING_OUT) {
                    ...
                    if (!dontquickfade)
                        ...
                        dontquickfade = false;
                } else if (Mix_FadeInMusicPos(musicTracks[t].m_music,
                                              -1,
                                              fadein_ms,
                                              position_sec) == -1) {
                    printf("Mix_FadeInMusicPos: %s\n", Mix_GetError());
...
{
    // Instead of returning early if music is muted, this should still increase
    // `musicVolume` so that anything that relies on this won't break. We'll
    // simply just not set the volume if the music is muted
...
void musicclass::niceplay(int t)
{
    ...
    if ((!mmmmmm && currentsong != t) ||
        ...
        (mmmmmm && !usingmmmmmm && currentsong != t + 16)) {
        if (currentsong != -1) {
            dontquickfade = true;
            fadeout();
        }
