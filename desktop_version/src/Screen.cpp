...
void Screen::init(int windowWidth,
                  int windowHeight,
                  bool fullscreen,
                  bool useVsync,
                  int stretch,
                  ...
                  bool badSignal)
{
    ...
    SDL_SetHintWithPriority(
        SDL_HINT_RENDER_VSYNC, vsync ? "1" : "0", SDL_HINT_OVERRIDE);
    ...
    SDL_Surface* icon = SDL_CreateRGBSurfaceFrom(data,
                                                 width,
                                                 height,
                                                 24,
                                                 width * 3,
    ...
    // ALSO FIXME: This SDL_CreateTexture() is duplicated in
...
void Screen::ResizeToNearestMultiple()
{
    ...
    if ((float)w / (float)h > 4.0 / 3.0) {
        ...
        // Height is bigger, so it's limited by width. Or we're exactly 4:3
        // already
        usethisdimension = w;
        usethisratio = 320;
        using_width = true;
...
void Screen::FlipScreen()
{
    ...
    SDL_RenderCopy(
        m_renderer, m_screenTexture, isFiltered ? &filterSubrect : NULL, NULL);
