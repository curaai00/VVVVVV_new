...
void gamelogic()
{
    ...
    if (game.deathseq != -1) {
        ...
        for (size_t i = 0; i < obj.entities.size(); i++) {
            ...
            if (obj.entities[i].type == 2 && obj.entities[i].state == 3) {
                // Ok! super magical exception for the room with the intention
                // death for the shiny trinket fix this when the maps are
                // finalised
                if (game.roomx != 111 || game.roomy != 107 || map.custommode) {
                ...
                // for the final level. probably something 99% of players won't
                // see.
                bool entitygone = false;
                while (obj.entities[i].state == 2) {
                    entitygone = obj.updateentities(i);
                ...
                if (!entitygone)
                    obj.entities[i].state = 4;
            } else if (obj.entities[i].type == 23 && game.swnmode &&
                       game.deathseq < 15) {
        ...
        if (game.swnmode) {
            // if playing SWN game a, push the clock back to the nearest 10
            // second interval
            if (game.swngame == 0) {
                game.swnpenalty();
        ...
        if (game.supercrewmate) {
            ...
            {
                int i = obj.getplayer();
                if (i > -1 && obj.entities[i].ax > 0 &&
                    obj.entities[i].xp > 280) {
                    obj.entities[i].ax = 0;
        ...
        if (game.swnmode) // which game?
        {
            ...
            } else if (game.swngame == 4) // create top line
            {
                game.swngame = 3;
                obj.createentity(
                    -8, 84 - 32, 11, 8); // (horizontal gravity line)
                music.niceplay(2);
        ...
        if (game.intimetrial) {
            ...
            if (game.timetrialcountdown > 0) {
                ...
                if (game.timetrialcountdown == 120)
                    music.playef(21);
                if (game.timetrialcountdown == 90)
                    music.playef(21);
                if (game.timetrialcountdown == 60)
                    music.playef(21);
                if (game.timetrialcountdown == 30) {
        ...
        if (!game.completestop) {
                ...
                for (int i = obj.entities.size() - 1; i >= 0; i--) {
                    if (obj.entities[i].isplatform) {
                        if (abs(obj.entities[i].vx) < 0.000001f) {
                            obj.removeblockat(obj.entities[i].xp,
                                              obj.entities[i].yp);
                ...
                for (int ie = obj.entities.size() - 1; ie >= 0; ie--) {
                    if (obj.entities[ie].isplatform) {
                        if (abs(obj.entities[ie].vy) < 0.000001f) {
                            obj.removeblockat(obj.entities[ie].xp,
                                              obj.entities[ie].yp);
            ...
            obj.entitycollisioncheck(); // Check ent v ent collisions, update
                                        // states

            if (map.towermode) {
                // special for tower: is the player touching any spike blocks?
                int player = obj.getplayer();
                if (player > -1 && obj.checktowerspikes(player) &&
                    graphics.fademode == 0) {
                    game.deathseq = 30;
                }
            }
        ...
        if (obj.customwarpmode) {
            // Rewritten system for mobile update: basically, the new logic is
            // to check if the player is leaving the map, and if so do a special
            // check against warp lines for collision
        ...
        if (map.warpx && !map.towermode) {
            for (size_t i = 0; i < obj.entities.size(); i++) {
                ...
                {
                    if (game.roomx == 118 && game.roomy == 102 &&
                        obj.entities[i].rule == 1 && !map.custommode) {
                        // ascii snakes
        ...
        if (!map.warpy && !map.towermode) {
            ...
            int player = obj.getplayer();
            if (player > -1 && game.door_down > -2 &&
                obj.entities[player].yp >= 238) {
                obj.entities[player].yp -= 240;
                map.gotoroom(game.roomx, game.roomy + 1);
                screen_transition = true;
            }
            if (player > -1 && game.door_up > -2 &&
                obj.entities[player].yp < -2) {
                obj.entities[player].yp += 240;
                map.gotoroom(game.roomx, game.roomy - 1);
                screen_transition = true;
            }
        ...
        if (map.custommode) {
            ...
            if (game.teleport) {
                if (game.roomx == 117 && game.roomy == 102) {
                    ...
                    // There are lots of warp tokens in this room, so we have to
                    // distinguish!
    ...
    if (game.roomchange) {
        ...
        if (game.companion > 0 && i > -1) {
            // ok, we'll presume our companion has been destroyed in the room
            // change. So:
            switch (game.companion) {
                case 6:
                    obj.createentity(
                        ...
                        121.0f,
                        15.0f,
                        1); // Y=121, the floor in that particular place!
                    j = obj.getcompanion();
                    if (j > -1) {
                    ...
                    if (game.roomy <= 105) // don't jump after him!
                    {
                        if (game.roomx == 110) {
                            obj.createentity(
                                ...
                                86,
                                16,
                                1); // Y=86, the ROOF in that particular place!
                        } else {
                            obj.createentity(
                                obj.entities[i].xp,
                                86.0f,
                ...
                case 8:
                    if (game.roomy >= 104) // don't jump after him!
                    {
                        if (game.roomx == 102) {
                            ...
                            obj.createentity(
                                obj.entities[i].xp, 177.0f, 17.0f, 1);
                            j = obj.getcompanion();
                            if (j > -1) {
                                obj.entities[j].vx = obj.entities[i].vx;
                                obj.entities[j].dir = obj.entities[i].dir;
                ...
                case 9:
                    if (!map.towermode) // don't go back into the tower!
                    {
                        if (game.roomx == 110 && obj.entities[i].xp < 20) {
                            ...
                            obj.createentity(
                                obj.entities[i].xp, 185.0f, 18.0f, 15, 0, 1);
                        }
                        j = obj.getcompanion();
                        if (j > -1) {
                            obj.entities[j].vx = obj.entities[i].vx;
                    ...
                    if (game.roomy == 51) {
                        if (!obj.flags[59]) {
                            obj.createentity(
                                ...
                                169.0f,
                                18,
                                graphics.crewcolour(game.lastsaved),
                                0,
                                10);
                            j = obj.getcompanion();
                            if (j > -1) {
    ...
    if (game.activetele && !game.advancetext && game.hascontrol &&
        !script.running && !game.intimetrial) {
        int i = obj.getplayer();
        if (i > -1) {
            obj.settemprect(i);
        }
