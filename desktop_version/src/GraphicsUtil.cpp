...
SDL_Surface* GetSubSurface(SDL_Surface* metaSurface,
                           int x,
                           int y,
                           int width,
                           int height)
{
    ...
    SDL_Surface* preSurface =
        SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, 32, r, g, b, a);
...
SDL_Surface* ScaleSurface(SDL_Surface* _surface,
                          int Width,
                          int Height,
                          SDL_Surface* Dest)
{
    ...
    SDL_Surface* _ret;
    if (Dest == NULL) {
        _ret = SDL_CreateRGBSurface(_surface->flags,
                                    Width,
                                    Height,
    ...
    float _stretch_factor_x =
              (static_cast<double>(Width) / static_cast<double>(_surface->w)),
          _stretch_factor_y =
              (static_cast<double>(Height) / static_cast<double>(_surface->h));

    SDL_Rect gigantoPixel;
    for (Sint32 y = 0; y < _surface->h; y++)
        for (Sint32 x = 0; x < _surface->w; x++) {
            setRect(gigantoPixel,
                    static_cast<Sint32>(float(x) * _stretch_factor_x),
                    static_cast<Sint32>(float(y) * _stretch_factor_y),
                    static_cast<Sint32>(_stretch_factor_x),
                    static_cast<Sint32>(_stretch_factor_y));
            SDL_FillRect(_ret, &gigantoPixel, ReadPixel(_surface, x, y));
        }

    // DrawPixel(_ret, static_cast<Sint32>(_stretch_factor_x * x) + o_x,
    // static_cast<Sint32>(_stretch_factor_y * y) + o_y, ReadPixel(_surface, x,
    // y));
...
{
    SDL_Surface* ret = SDL_CreateRGBSurface(_src->flags,
                                            _src->w,
                                            _src->h,
...
void BlitSurfaceStandard(SDL_Surface* _src,
                         SDL_Rect* _srcRect,
                         SDL_Surface* _dest,
                         SDL_Rect* _destRect)
{
...
void BlitSurfaceTinted(SDL_Surface* _src,
                       SDL_Rect* _srcRect,
                       SDL_Surface* _dest,
                       SDL_Rect* _destRect,
                       colourTransform& ct)
{
    ...
    for (int x = 0; x < tempsurface->w; x++) {
        for (int y = 0; y < tempsurface->h; y++) {
            ...
            double gray =
                floor((temp_pixred + temp_pixgreen + temp_pixblue + 0.5));

            Uint8 ctred =
                (ct.colour & graphics.backBuffer->format->Rmask) >> 16;
            Uint8 ctgreen =
                (ct.colour & graphics.backBuffer->format->Gmask) >> 8;
            Uint8 ctblue =
                (ct.colour & graphics.backBuffer->format->Bmask) >> 0;
...
{
    SDL_Surface* _ret = SDL_CreateRGBSurface(_src->flags,
                                             _src->w,
                                             _src->h,
                                             32,
                                             _src->format->Rmask,
    ...
    for (int x = 0; x < _src->w; x++) {
        for (int y = 0; y < _src->h; y++) {
            ...
            Uint32 pixelOffset =
                ReadPixel(_src, std::min(x + redOffset, 319), sampley);
            ...
            Uint32 finalPixel = ((red << 16) + (green << 8) + (blue << 0)) |
                                (pixel & _src->format->Amask);
...
void FillRect(SDL_Surface* _surface,
              const int _x,
              const int _y,
              const int _w,
              const int _h,
...
{
    auto is_scrollable = [](int _pX, int _pY) -> bool {
        return _pY < 0 || _pY > 0 || _pX <= 0 || _pX > 0;
    };
