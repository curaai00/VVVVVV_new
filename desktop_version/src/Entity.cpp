...
bool entityclass::checktowerspikes(int t)
{
    ...
    if (map.spikecollide(tempx, tempy))
        return true;
    if (map.spikecollide(tempw, tempy))
        return true;
    if (map.spikecollide(tempx, temph))
        return true;
    if (map.spikecollide(tempw, temph))
        return true;
    if (temprect.h >= 12) {
        tpy1 = getgridpoint(temprect.y + 6);
        if (map.spikecollide(tempx, tpy1))
            return true;
        if (map.spikecollide(tempw, tpy1))
            return true;
        if (temprect.h >= 18) {
...
void entityclass::gravcreate(int ypos,
                             int dir,
                             int xoff /*= 0*/,
                             int yoff /*= 0*/)
{
        ...
        if (t == 0) // game 0, survive for 30 seconds
        {
            switch (game.swnstate) {
                case 0:
                    ...
                    if (game.deathcounts - game.swndeaths > 7)
                        game.swndelay += 2;
                    if (game.deathcounts - game.swndeaths > 15)
                        game.swndelay += 2;
                    if (game.deathcounts - game.swndeaths > 25)
                        game.swndelay += 4;
                    break;
                case 1:
                    createentity(
                        -150, 58 + (int(fRandom() * 6) * 20), 23, 0, 0);
                    game.swnstate = 0;
                ...
                case 2:
                    ...
                    createentity(
                        -150, 58 + (int(game.swnstate2) * 20), 23, 0, 0);
                    game.swnstate = 0;
                    game.swndelay = 0; // return to decision state
                    break;
                case 3:
                    createentity(
                        320 + 150, 58 + (int(fRandom() * 6) * 20), 23, 1, 0);
                    game.swnstate = 0;
                    game.swndelay = 0; // return to decision state
                    break;
                case 4:
                    ...
                    createentity(
                        320 + 150, 58 + ((5 - game.swnstate2) * 20), 23, 1, 0);
                    game.swnstate = 0;
                    game.swndelay = 0; // return to decision state
                    game.swnstate2 = 0;
                    break;
                ...
                case 9:
                    ...
                    createentity(
                        320 + 150, 58 + (int(game.swnstate2) * 20), 23, 1, 0);
                    game.swnstate = 0;
                    game.swndelay = 0; // return to decision state
                    break;
            }
...
void entityclass::createblock(int t,
                              int xp,
                              int yp,
                              int w,
                              int h,
                              ...
                              const std::string& script /*= ""*/)
{
    ...
    switch (t) {
        ...
        case ACTIVITY: // Activity Zone
            ...
            // Ok, each and every activity zone in the game is initilised here.
            // "Trig" in this case is a variable that assigns all the details.
            switch (trig) {
                case 0: // testing zone
...
void entityclass::removeblockat(int x, int y)
{
    for (size_t i = 0; i < blocks.size(); i++) {
        if (blocks[i].xp == int(x) && blocks[i].yp == int(y))
            removeblock_iter(i);
    }
}
...
{
    if (t < 0 || t > (int)entities.size() || s < 0 ||
        s > (int)linecrosskludge.size()) {
...
bool entityclass::gridmatch(int p1,
                            int p2,
                            int p3,
                            int p4,
                            int p11,
                            ...
                            int p41)
{
    if (p1 == p11 && p2 == p21 && p3 == p31 && p4 == p41)
        return true;
    return false;
}
...
void entityclass::createentity(float xp,
                               float yp,
                               int t,
                               float vx /*= 0*/,
                               float vy /*= 0*/,
                               ...
                               int p4 /*= 240 */)
{
    ...
    // Rule 3 is anything that results in an entity to entity collision and
    // state change Rule 4 is a horizontal line, 5 is vertical Rule 6 is a crew
    // member

#if !defined(NO_CUSTOM_LEVELS)
    // Special case for gray Warp Zone tileset!
    int room = game.roomx - 100 + (game.roomy - 100) * ed.maxwidth;
    bool custom_gray = room >= 0 && room < 400 && ed.level[room].tileset == 3 &&
                       ed.level[room].tilecol == 6;
#else
    ...
    switch (t) {
        ...
        case 1: // Simple enemy, bouncing off the walls
            ...
            } else if (game.roomx == 113 &&
                       (game.roomy <= 110 && game.roomy >= 108)) {
                entity.setenemy(1);
                entity.setenemyroom(game.roomx, game.roomy); // For colour
        ...
        case 2: // A moving platform
            ...
            if (customplatformtile > 0) {
                ...
                if (gridmatch(p1, p2, p3, p4, 100, 70, 320, 160))
                    entity.tile = 616;
                if (gridmatch(p1, p2, p3, p4, 72, 0, 248, 240))
                    entity.tile = 610;
                if (gridmatch(p1, p2, p3, p4, -20, 0, 320, 240))
                    entity.tile = 413;

                if (gridmatch(p1, p2, p3, p4, -96, -72, 400, 312))
                    entity.tile = 26;
                if (gridmatch(p1, p2, p3, p4, -32, -40, 352, 264))
                    entity.tile = 27;
            }
            ...
            if (int(vx) <= 1)
                vertplatforms = true;
            if (int(vx) >= 2 && int(vx) <= 5)
                horplatforms = true;
            if (int(vx) == 14 || int(vx) == 15)
                horplatforms =
                    true; // special case for last part of Space Station
            if (int(vx) >= 6 && int(vx) <= 7)
                vertplatforms = true;
        ...
        case 3: // Disappearing platforms
            ...
            if (customplatformtile > 0) {
                ...
                if (game.roomx == 49 && game.roomy == 52)
                    entity.tile = 18;
                if (game.roomx == 50 && game.roomy == 52)
                    entity.tile = 22;
            }
        ...
        case 8: // Small collectibles
            ...
            entity.para = vx;
            if (!INBOUNDS_ARR(vx, collect) || collect[(int)vx])
                return;
            break;
        ...
        case 22: // Fake trinkets, only appear if you've collected them
            ...
            break;
        case 23: // SWN Enemies
            // Given a different behavior, these enemies are especially for SWN
            // mode and disappear outside the screen.
            entity.rule = 1;
            entity.type = 23;
            entity.behave = vx;
            entity.para = vy;
        ...
        case 24: // Super Crew Member
            // This special crewmember is way more advanced than the usual kind,
            // and can interact with game objects
            entity.rule = 6;
        ...
        case 25: // Trophies
            ...
            // Decide tile here based on given achievement: both whether you
            // have them and what they are default is just a trophy base:
            entity.tile = 180 + vx;
            switch (int(vy)) {
                case 1:
        ...
        case 55: // Crew Member (custom, collectable)
            ...
            // Check if it's already been collected
            entity.para = vx;
            if (!INBOUNDS_ARR(vx, customcollect) || customcollect[(int)vx])
                return;
            break;
    ...
    if (entity.type == 1) {
        switch (entity.animate) {
            ...
            case 7:  // Simpler Loop (just two frames) (slower) (with
                     // directions!)
            case 11: // Conveyor right
                entity.drawframe++;
                break;
            case 3: // Simpler Loop (just two frames, but double sized)
            case 4: // Simpler Loop (just two frames, but double sized) (as
                    // above, but slower)
        ...
        switch (entities[i].type) {
                ...
                switch (entities[i].behave) {
                    case 0:                         // Bounce, Start moving down
                        if (entities[i].state == 0) // Init
                        {
                            ...
                            if (entities[i].outside())
                                entities[i].state = entities[i].onwall;
                    ...
                    case 5: // Always move right
                        if (entities[i].state == 0) {
                            ...
                            entities[i].onwall = 0;
                            entities[i].xp -=
                                static_cast<int>(entities[i].para);
                    ...
                    case 10:
                        ...
                        if (entities[i].state == 0) {
                            createentity(
                                entities[i].xp + 28, entities[i].yp, 1, 10, 1);
                            entities[i].state = 1;
                    ...
                    case 12:
                        ...
                        if (entities[i].state == 0) {
                            createentity(
                                entities[i].xp, entities[i].yp, 1, 12, 1);
                            entities[i].state = 1;
                    ...
                    case 14: // Very special hack: as two, but doesn't move in
                        ...
                        if (entities[i].state == 0) // Init
                        {
                            for (size_t j = 0; j < entities.size(); j++) {
                                if (entities[j].type == 2 &&
                                    entities[j].state == 3 &&
                                    entities[j].xp == (entities[i].xp - 32)) {
                                    entities[i].state = 3;
                                    bool entitygone = updateentities(i);
                                    if (entitygone)
                                        return true;
            ...
            case 10: // Vertical gravity Lines
                if (entities[i].state == 1) {
                    ...
                    // Special case for room initilisations: As state one,
                    // except without the reversal
                    entities[i].onentity = 3;
                    entities[i].state = 2;
                }
                break;
            case 11: // Warp point
                // wait for collision
                if (entities[i].state == 1) {
                    // Depending on the room the warp point is in, teleport to a
                    // new location!
                    entities[i].onentity = 0;
                    ...
                    if (int(entities[i].xp) == 12 * 8)
                        game.teleportxpos = 1;
                    if (int(entities[i].xp) == 5 * 8)
                        game.teleportxpos = 2;
                    if (int(entities[i].xp) == 28 * 8)
                        game.teleportxpos = 3;
                    if (int(entities[i].xp) == 21 * 8)
                        game.teleportxpos = 4;
                }
                break;
            case 12: // Crew member
                // Somewhat complex AI: exactly what they do depends on room,
                // location, state etc At state 0, do nothing at all.
                if (entities[i].state == 1) {
                    // happy!
                    if (entities[k].rule == 6)
                        entities[k].tile = 0;
                    if (entities[k].rule == 7)
                        entities[k].tile = 6;
                    ...
                    // 11-15 means to follow a specific character, in crew order
                    // (cyan, purple, yellow, red, green, blue)
            ...
            case 14: // Super Crew member
                ...
                if (entities[i].state == 0) {
                    ...
                    if (j > -1 && entities[j].onground > 0) {
                        if (entities[j].xp > entities[i].xp + 5) {
                            entities[i].dir = 1;
                        } else if (entities[j].xp > 15 &&
                                   entities[j].xp < entities[i].xp - 5) {
                            entities[i].dir = 0;
                        }
            ...
            case 100: // The teleporter
                if (entities[i].state == 1) {
                    ...
                    // Initilise the teleporter without changing the game state
                    // or playing sound
                    entities[i].onentity = 0;
                    entities[i].tile = 6;
    ...
    if (entities[_i].statedelay < 1) {
        switch (entities[_i].type) {
            ...
            case 23:
                ...
                switch (entities[_i].animate) {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        // Simpler Loop (just two frames, but double sized) (as
                        // above but slower)
                        entities[_i].framedelay--;
                        if (entities[_i].framedelay <= 0) {
                    ...
                    case 6:
                        ...
                        break;
                    case 7:
                        // Simpler Loop (just two frames) (slower) (with
                        // directions!)
                        entities[_i].framedelay--;
                        if (entities[_i].framedelay <= 0) {
                            entities[_i].framedelay = 6;
            ...
            case 14: // Crew member! Very similar to hero
                ...
                if (entities[_i].onground > 0 || entities[_i].onroof > 0) {
                    if (entities[_i].vx > 0.0000f ||
                        entities[_i].vx < -0.000f) {
                        // Walking
                        if (entities[_i].framedelay <= 0) {
                            entities[_i].framedelay = 4;
                            entities[_i].walkingframe++;
                ...
                if (game.deathseq > -1) {
                    ...
                    if (entities[_i].rule == 7)
                        entities[_i].drawframe += 2;
                    // if (game.gravitycontrol == 1) entities[_i].drawframe +=
                    // 2;
                }
                break;
            case 100: // the teleporter!
        ...
        if (entities[_i].statedelay < 0)
            entities[_i].statedelay = 0;
    }
}
...
{
    if (a < 0 || a > (int)entities.size() || b < 0 ||
        b > (int)entities.size()) {
        ...
        if (entities[i].rule == 0) {
            ...
            for (size_t j = 0; j < blocks.size(); j++) {
                if (blocks[j].type == DAMAGE &&
                    help.intersects(blocks[j].rect, temprect)) {
                    return true;
                }
            }
        }
...
{
    // Return true if rectset intersects a moving platform, setups px & py to
    // the platform x & y
    for (size_t i = 0; i < blocks.size(); i++) {
        if (blocks[i].type == BLOCK &&
            help.intersects(blocks[i].rect, temprect)) {
            px = blocks[i].xp;
            py = blocks[i].yp;
            return true;
        }
...
{
    if (t < 0 || t >= (int)entities.size() || l < 0 ||
        l >= (int)entities.size()) {
    ...
    if (entities[t].xp + entities[t].cx + entities[t].w >= entities[l].xp) {
        if (entities[t].xp + entities[t].cx <= entities[l].xp + entities[l].w) {
            ...
            linetemp +=
                yline(entities[t].oldyp + entities[t].h, entities[l].yp);

            if (linetemp > -4 && linetemp < 4)
                return true;
            return false;
        }
    }
...
bool entityclass::entityvlinecollide(int t, int l)
{
    ...
    if (entities[t].yp + entities[t].cy + entities[t].h >= entities[l].yp &&
        ...
        linetemp += yline(entities[t].xp + entities[t].cx + 1 + entities[t].w,
                          entities[l].xp);
        linetemp +=
            yline(entities[t].oldxp + entities[t].cx + 1, entities[l].xp);
        linetemp +=
            yline(entities[t].oldxp + entities[t].cx + 1 + entities[t].w,
                  entities[l].xp);
...
bool entityclass::entitywarphlinecollide(int t, int l)
{
    ...
    if (entities[t].xp + entities[t].cx + entities[t].w >= entities[l].xp &&
        ...
        if (entities[l].yp < 120) {
            // Top line
            if (entities[t].vy < 0) {
                if (entities[t].yp < entities[l].yp + 10)
                    linetemp++;
                if (entities[t].yp + entities[t].h < entities[l].yp + 10)
                    linetemp++;
                if (entities[t].oldyp < entities[l].yp + 10)
                    linetemp++;
                if (entities[t].oldyp + entities[t].h < entities[l].yp + 10)
                    linetemp++;
            }
...
bool entityclass::entitywarpvlinecollide(int t, int l)
{
    ...
    if (entities[t].yp + entities[t].cy + entities[t].h >= entities[l].yp &&
        ...
        if (entities[l].xp < 160) {
            // Left hand line
            if (entities[t].xp + entities[t].cx + 1 < entities[l].xp + 10)
                linetemp++;
            if (entities[t].xp + entities[t].cx + 1 + entities[t].w <
                entities[l].xp + 10)
                linetemp++;
            if (entities[t].oldxp + entities[t].cx + 1 < entities[l].xp + 10)
                linetemp++;
            if (entities[t].oldxp + entities[t].cx + 1 + entities[t].w <
                entities[l].xp + 10)
                linetemp++;
...
void entityclass::fixfriction(int t, float xfix, float xrate, float yrate)
{
    ...
    if (std::abs(entities[t].vx - xfix) <= xrate)
        entities[t].vx = xfix;
    if (std::abs(entities[t].vy) < yrate)
        entities[t].vy = 0;
}
...
void entityclass::applyfriction(int t, float xrate, float yrate)
{
    ...
    if (entities[t].vx > 0.00f)
        entities[t].vx -= xrate;
    if (entities[t].vx < 0.00f)
        entities[t].vx += xrate;
    if (entities[t].vy > 0.00f)
        entities[t].vy -= yrate;
    if (entities[t].vy < 0.00f)
        entities[t].vy += yrate;
    if (entities[t].vy > 10.00f)
        entities[t].vy = 10.0f;
    if (entities[t].vy < -10.00f)
        entities[t].vy = -10.0f;
    if (entities[t].vx > 6.00f)
        entities[t].vx = 6.0f;
    if (entities[t].vx < -6.00f)
        entities[t].vx = -6.0f;

    if (std::abs(entities[t].vx) < xrate)
        entities[t].vx = 0.0f;
    if (std::abs(entities[t].vy) < yrate)
        entities[t].vy = 0.0f;
}
        ...
        if (entitycollide(t, j)) {
            ...
            if (testwallsy(j, entities[j].xp, entities[j].newyp)) {
                if (entities[t].vy > 0) {
                    ...
                    entities[j].yp =
                        entities[t].yp - entities[j].h - entities[j].cy;
                    entities[j].vy = 0;
...
void entityclass::scmmovingplatformfix(int t)
{
    ...
    if (entitycollide(t, j)) {
        ...
        if (entitycollide(t, j)) {
            ...
            entities[j].vy = entities[t].vy;
            entities[j].newyp =
                static_cast<float>(entities[j].yp) + entities[j].vy;
            if (testwallsy(j, entities[j].xp, entities[j].newyp)) {
                if (entities[t].vy > 0) {
                    entities[j].yp = entities[t].yp + entities[t].h;
                    entities[j].vy = 0;
...
void entityclass::hormovingplatformfix(int t)
{
    ...
    createblock(
        0, entities[t].xp, entities[t].yp, entities[t].w, entities[t].h);
}
...
void entityclass::customwarplinecheck(int i)
{
    ...
    // Turns on obj.customwarpmodevon and obj.customwarpmodehon if player
    // collides with warp lines

    // We test entity to entity
    for (int j = 0; j < (int)entities.size(); j++) {
        if (i != j) {
            if (entities[i].rule == 0 &&
                entities[j].rule == 5 // Player vs vertical line!
            ...
            if (entities[i].rule == 0 &&
                entities[j].rule == 7 // Player vs horizontal WARP line
...
void entityclass::entitycollisioncheck()
{
    std::vector<SDL_Surface*>& spritesvec =
        graphics.flipmode ? graphics.flipsprites : graphics.sprites;

    for (size_t i = 0; i < entities.size(); i++) {
        // We test entity to entity
        for (size_t j = 0; j < entities.size(); j++) {
            if (i != j) {
                if (entities[i].rule == 0 && entities[j].rule == 1 &&
                    entities[j].harmful) {
                    // player i hits enemy or enemy bullet j
                    if (entitycollide(i, j) && !map.invincibility) {
                        if (entities[i].size == 0 &&
                            (entities[j].size == 0 || entities[j].size == 12)) {
                            ...
                            if (INBOUNDS(drawframe1, spritesvec) &&
                                INBOUNDS(drawframe2, spritesvec) &&
                                graphics.Hitest(spritesvec[drawframe1],
                                                colpoint1,
                                                spritesvec[drawframe2],
                                                colpoint2)) {
                                // Do the collision stuff
                                game.deathseq = 30;
                            }
                        } else {
                            // Ok, then we just assume a normal bounding box
                            // collision
                            game.deathseq = 30;
                        }
                    }
                ...
                if (entities[i].rule == 0 &&
                    entities[j].rule == 2) // Moving platforms
                {
                    if (entitycollide(i, j))
                        removeblockat(entities[j].xp, entities[j].yp);
                }
                if (entities[i].rule == 0 &&
                    entities[j].rule == 3) // Entity to entity
                {
                ...
                if (entities[i].rule == 0 &&
                    entities[j].rule == 4) // Player vs horizontal line!
                {
                    if (game.deathseq == -1) {
                        // Here we compare the player's old position versus his
                        // new one versus the line. All points either be above or
                        // below it. Otherwise, there was a collision this frame.
                        if (entities[j].onentity > 0) {
                            if (entityhlinecollide(i, j)) {
                                music.playef(8);
                                game.gravitycontrol =
                                    (game.gravitycontrol + 1) % 2;
                                game.totalflips++;
                                if (game.gravitycontrol == 0) {
                                    if (entities[i].vy < 1)
                                        entities[i].vy = 1;
                ...
                if (entities[i].rule == 0 &&
                    entities[j].rule ==
                        6) // Player versus crumbly blocks! Special case
                {
                    if (entities[j].onentity > 0) {
                        // ok; only check the actual collision if they're in a
                        // close proximity
                ...
                if (game.supercrewmate) {
                    ...
                    {
                        if (entities[j].rule == 1 &&
                            entities[j].harmful) // j is a harmful enemy
                        {
                            // player i hits enemy or enemy bullet j
                            if (entitycollide(i, j) && !map.invincibility) {
                                if (entities[i].size == 0 &&
                                    entities[j].size == 0) {
                                    // They're both sprites, so do a per pixel
                                    // collision
                                    colpoint1.x = entities[i].xp;
                                    colpoint1.y = entities[i].yp;
                                    colpoint2.x = entities[j].xp;
                                    ...
                                    // Ok, then we just assume a normal bounding
                                    // box collision
                                    game.deathseq = 30;
                                    game.scmhurt = true;
                                }
                        ...
                        if (entities[j].type == 8 &&
                            entities[j].rule == 3) // Entity to entity (well,
                                                   // checkpoints anyway!)
                        {
                            if (entities[j].onentity > 0) {
                                if (entitycollide(i, j))
    ...
    // WARNING: If updating this code, don't forget to update Map.cpp
    ...
    if (checktrigger(&block_idx) > -1 && block_idx > -1) {
        ...
        if (blocks[block_idx].script != "" &&
            (activetrigger < 300 || activetrigger > 336)) {
