...
void entityclass::resetallflags() { SDL_memset(flags, false, sizeof(flags)); }
                              ...
                              const std::string &script /*= ""*/)
{
    ...
    case ACTIVITY: // Activity Zone
        ...
        // Ok, each and every activity zone in the game is initilised here. "Trig" in
        // this case is a variable that assigns all the details.
        switch (trig)
        {
        case 0: // testing zone
...
void entityclass::removeallblocks() { blocks.clear(); }
...
bool entityclass::gridmatch(int p1, int p2, int p3, int p4, int p11, int p21,
                            int p31, int p41)
{
...
void entityclass::createentity(float xp, float yp, int t, float vx /*= 0*/,
                               float vy /*= 0*/, int p1 /*= 0*/, int p2 /*= 0*/,
                               int p3 /*= 320*/, int p4 /*= 240 */)
{
    k = entities.size();
    ...
    // Rule 3 is anything that results in an entity to entity collision and state
    // change Rule 4 is a horizontal line, 5 is vertical Rule 6 is a crew member
    ...
    case 2: // A moving platform
        ...
        if (int(vx) == 14 || int(vx) == 15)
            horplatforms = true; // special case for last part of Space Station
    ...
    case 22: // Fake trinkets, only appear if you've collected them
        ...
        break;
    case 23: // SWN Enemies
        // Given a different behavior, these enemies are especially for SWN mode and
    ...
    case 24: // Super Crew Member
        // This special crewmember is way more advanced than the usual kind, and can
    ...
    case 25: // Trophies
        ...
        // Decide tile here based on given achievement: both whether you have them
        // and what they are default is just a trophy base:
        entity.tile = 180 + vx;
        switch (int(vy))
        {
        case 1:
    ...
    if (entity.type == 1)
    {
        ...
        case 4: // Simpler Loop (just two frames, but double sized) (as above, but
                // slower)
    ...
    if (entities[i].statedelay <= 0)
    {
        ...
        case 1: // Movement behaviors
            ...
            case 13: // Always move up, destroy when outside screen
                ...
                break;
            case 14: // Very special hack: as two, but doesn't move in specific
                     // circumstances
                if (entities[i].state == 0) // Init
                {
                    for (size_t j = 0; j < entities.size(); j++)
                    {
                        if (entities[j].type == 2 && entities[j].state == 3 &&
            ...
            break;
        case 2: // Disappearing platforms //wait for collision
        ...
        case 10: // Vertical gravity Lines
            ...
            {
                // Special case for room initilisations: As state one, except without
                // the reversal
                entities[i].onentity = 3;
                entities[i].state = 2;
            }
        ...
        case 11: // Warp point
            ...
            if (entities[i].state == 1)
            {
                // Depending on the room the warp point is in, teleport to a new
                // location!
                entities[i].onentity = 0;
            ...
            break;
        case 12: // Crew member
            // Somewhat complex AI: exactly what they do depends on room, location,
            // state etc At state 0, do nothing at all.
            if (entities[i].state == 1)
            {
                // happy!
            ...
            {
                // 11-15 means to follow a specific character, in crew order (cyan,
        ...
        case 100: // The teleporter
            ...
            {
                // Initilise the teleporter without changing the game state or
                // playing sound
                entities[i].onentity = 0;
                entities[i].tile = 6;
    ...
    if (entities[_i].statedelay < 1)
    {
        ...
        case 23:
            ...
            case 3:
                ...
                break;
            case 4:
                // Simpler Loop (just two frames, but double sized) (as above but
                // slower)
                entities[_i].framedelay--;
                if (entities[_i].framedelay <= 0)
                {
...
{
    // Return true if rectset intersects a moving platform, setups px & py to the
    // platform x & y
    for (size_t i = 0; i < blocks.size(); i++)
    {
...
bool entityclass::entityvlinecollide(int t, int l)
{
        ...
        entities[t].yp + entities[t].cy <= entities[l].yp + entities[l].h)
    {
        ...
        linetemp += yline(entities[t].oldxp + entities[t].cx + 1 + entities[t].w,
                          entities[l].xp);
        ...
        for (size_t j = 0; j < entities.size(); j++)
        {
            if (i != j)
            {
                    ...
                    if (entitycollide(i, j) && !map.invincibility)
                    {
                            ...
                            (entities[j].size == 0 || entities[j].size == 12))
                        {
                            ...
                            if (INBOUNDS(drawframe1, spritesvec) &&
                                ...
                                graphics.Hitest(spritesvec[drawframe1], colpoint1,
                                                spritesvec[drawframe2], colpoint2))
                            {
                ...
                if (entities[j].rule == 4) // Player vs horizontal line!
                {
                    if (game.deathseq == -1)
                    {
                        // Here we compare the player's old position versus his new
                        // one versus the line. All points either be above or below
                        // it. Otherwise, there was a collision this frame.
                        if (entities[j].onentity > 0)
                        {
                    ...
                    6) // Player versus crumbly blocks! Special case
                {
                    if (entities[j].onentity > 0)
                    {
                        // ok; only check the actual collision if they're in a close
                        // proximity
                ...
                if (entities[j].rule ==
                    7) // Player versus horizontal warp line, pre-2.1
                {
                    if (game.glitchrunnermode && game.deathseq == -1 &&
                        entities[j].onentity > 0 && entityhlinecollide(i, j))
                    {
                        entities[j].state = entities[j].onentity;
                    }
                }
            ...
            if (entities[i].type == 14) // i is the supercrewmate
            {
                for (size_t j = 0; j < entities.size(); j++)
                {
                    if (i != j)
                    {
                            ...
                            if (entitycollide(i, j) && !map.invincibility)
                            {
                                if (entities[i].size == 0 && entities[j].size == 0)
                                {
                                    ...
                                    if (INBOUNDS(drawframe1, spritesvec) &&
                                        INBOUNDS(drawframe2, spritesvec) &&
                                        graphics.Hitest(
                                            spritesvec[drawframe1], colpoint1,
                                            spritesvec[drawframe2], colpoint2))
                                    {
                                        // Do the collision stuff
                                        game.deathseq = 30;
                        ...
                        if (entities[j].type == 8 &&
                            entities[j].rule ==
                                3) // Entity to entity (well, checkpoints anyway!)
                        {
                            if (entities[j].onentity > 0)
                            {
                                if (entitycollide(i, j))
