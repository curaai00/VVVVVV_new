...
int FILESYSTEM_init(char* argvZero, char* baseDir, char* assetsPath)
{
    ...
    if (baseDir && baseDir[0] != '\0') {
        ...
        bool trailing_pathsep =
            SDL_strcmp(baseDir + SDL_strlen(baseDir) - SDL_strlen(pathSep),
                       pathSep) == 0;
    ...
    if (assetsPath) {
        ...
        SDL_snprintf(
            output, sizeof(output), "%s%s", PHYSFS_getBaseDir(), "data.zip");
    }
...
void FILESYSTEM_mountassets(const char* path)
{
    ...
    std::string zippath =
        "levels/" + _path.substr(7, _path.size() - 14) + ".data.zip";
    ...
    } else if (zip_path != "data.zip" && !endsWith(zip_path, "/data.zip") &&
               endsWith(zip_path, ".zip")) {
...
void FILESYSTEM_loadFileToMemory(const char* name,
                                 unsigned char** mem,
                                 size_t* len,
                                 bool addnull)
{
    if (strcmp(name, "levels/special/stdin.vvvvvv") == 0) {
        // this isn't *technically* necessary when piping directly from a file,
        // but checking for that is annoying
        ...
        if (!STDIN_LOADED) {
            ...
            STDIN_BUFFER.push_back(
                0); // there's no observable change in behavior if addnull is
                    // always true, but not vice versa
            STDIN_LOADED = true;
        }
        ...
        *mem = static_cast<unsigned char*>(
            malloc(length)); // STDIN_BUFFER.data() causes double-free
        std::copy(STDIN_BUFFER.begin(),
                  STDIN_BUFFER.end(),
...
bool FILESYSTEM_saveTiXml2Document(const char* name, tinyxml2::XMLDocument& doc)
{
    ...
    PHYSFS_writeBytes(
        ...
        printer.CStr(),
        printer.CStrSize() -
            1); // subtract one because CStrSize includes terminating null
    PHYSFS_close(handle);
    return true;
}
...
void PLATFORM_getOSDirectory(char* output)
{
    ...
    SHGetFolderPathW(
        NULL, CSIDL_PERSONAL, NULL, SHGFP_TYPE_CURRENT, utf16_path);
    WideCharToMultiByte(
        CP_UTF8, 0, utf16_path, -1, output, MAX_PATH, NULL, NULL);
    SDL_strlcat(output, "\\VVVVVV\\", MAX_PATH);
#else
    SDL_strlcpy(
        output, PHYSFS_getPrefDir("distractionware", "VVVVVV"), MAX_PATH);
#endif
}
...
#undef COPY_SAVEFILE
        ...
        else if (strcmp(de->d_name, "Saves") == 0)
        {
            ...
            for (subDe = readdir(subDir); subDe != NULL;
    ...
    {                                                                          \
        ...
        PLATFORM_copyFile(oldLocation, newLocation);                           \
                ...
                if COPY_SAVEFILE ("unlock.vvv")
                    else if COPY_SAVEFILE ("tsave.vvv") else if COPY_SAVEFILE (
                        "qsave.vvv")
#undef COPY_SAVEFILE
            }
        }
...
{
    pid_t child;
    // This const_cast is legal (ctrl-f "The statement" at
    // https://pubs.opengroup.org/onlinepubs/9699919799/functions/exec.html
    char* argv[3] = { const_cast<char*>(open_cmd),
