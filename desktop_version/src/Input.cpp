...
typedef SDL_GameControllerButton CtrlBtn;

void updatebuttonmappings(int bind)
{
    auto erase_ctrl_btn = [](CtrlBtn k,
                             std::vector<CtrlBtn>& btn_list) -> bool {
        auto find_res = std::find(btn_list.begin(), btn_list.end(), k);
        bool is_found = find_res != btn_list.end();
        if (is_found)
            btn_list.erase(find_res);
        return is_found;
    };
    std::vector<SDL_GameControllerButton>* btn_list_arr[4] = {
    ...
    for (SDL_GameControllerButton i = SDL_CONTROLLER_BUTTON_A;
        ...
        if (key.isDown(i)) {
            bool dupe = false;
            if (0 < bind && bind < 5) {
                for (size_t j = 0; j < btn_list_arr[bind]->size(); j += 1) {
                    if (i == (*btn_list_arr[bind])[j]) {
                        dupe = true;
                    }
                }
                if (!dupe) {
                    btn_list_arr[bind]->push_back(i);
                    music.playef(11);
                }
                for (int k = 0; k < 4; k++) {
                    if (k != bind)
                        erase_ctrl_btn(i, *btn_list_arr[k]);
                }
            }
        }
    }
...
#if !defined(NO_CUSTOM_LEVELS)
        case Menu::levellist:
            if (game.menu_.getCurOptIdx() ==
                (int)game.menu_.getCurOptSize() - 1) {
                // go back to menu
                music.playef(11);
                game.menu_.back();
                map.nexttowercolour();
            } else if (game.menu_.getCurOptIdx() ==
                       (int)game.menu_.getCurOptSize() - 2) {
                // previous page
                music.playef(11);
                if (game.levelpage == 0) {
                ...
                game.menu_.setCurOptIdx(game.menu_.getCurOptSize() - 2);
                map.nexttowercolour();
            } else if (game.menu_.getCurOptIdx() ==
                       (int)game.menu_.getCurOptSize() - 3) {
                // next page
                music.playef(11);
                if ((size_t)((game.levelpage * 8) + 8) >=
                    ed.ListOfMetaData.size()) {
                    game.levelpage = 0;
                } else {
                    game.levelpage++;
                }
                game.createmenu(Menu::levellist, true);
                game.menu_.setCurOptIdx(game.menu_.getCurOptSize() - 3);
                map.nexttowercolour();
            } else {
                // Ok, launch the level!
                // PLAY CUSTOM LEVEL HOOK
                music.playef(11);
                game.playcustomlevel =
                    (game.levelpage * 8) + game.menu_.getCurOptIdx();
                game.customleveltitle =
                    ed.ListOfMetaData[game.playcustomlevel].title;
                game.customlevelfilename =
                    ed.ListOfMetaData[game.playcustomlevel].filename;

                std::string name =
                    "saves/" +
                    ed.ListOfMetaData[game.playcustomlevel].filename.substr(7) +
                    ".vvv";
...
#define OFFSET 0
            ...
            switch (game.menu_.getCurOptIdx()) {
                case 0:
                    ...
                    game.levelpage = 0;
                    ed.getDirectoryData();
                    game.loadcustomlevelstats(); // Should only load a file if
                                                 // it's needed
...
#undef OFFSET
        ...
        case Menu::graphicoptions:
            switch (game.menu_.getCurOptIdx()) {
                case 0:
                    ...
                    game.createmenu(game.menu_.getCurOptName(), true);
                    game.menu_.setCurOptIdx(0);
                    break;
                case 1:
                    music.playef(11);
        ...
        case Menu::youwannaquit:
            switch (game.menu_.getCurOptIdx()) {
                ...
                default:
                    music.playef(11);
                    map.nexttowercolour(); // This goes first, otherwise
                                           // mismatching continuemenu color is
                                           // possible
                    game.menu_.back();
            }
            break;
        ...
        case Menu::options: {
            ...
            switch (game.menu_.getCurOptIdx()) {
                ...
                case 2:
...
#endif
                    {
                        ...
                        // Fix wrong area music in Tower (Positive Force vs.
                        // ecroF evitisoP)
                        if (map.custommode) {
                            break;
                        }
            ...
            if (game.menu_.getCurOptIdx() == 7 + offset) {
                // back
                music.playef(11);
                if (game.ingame_titlemode) {
                    game.returntopausemenu();
        ...
        case Menu::credits3:
            switch (game.menu_.getCurOptIdx()) {
                case 0:
                    ...
                    if (game.current_credits_list_index >=
                        (int)SDL_arraysize(Credits::superpatrons)) {
                        // No more super patrons. Move to the next credits
                        // section
                        game.current_credits_list_index = 0;
                        game.createmenu(Menu::credits4, true);
                    } else {
                        // There are more super patrons. Refresh the menu with
                        // the next ones
                        game.createmenu(Menu::credits3, true);
                    }
                ...
                case 1:
                    ...
                    if (game.current_credits_list_index < 0) {
                        // No more super patrons. Move to the previous credits
                        // section
                        game.current_credits_list_index = 0;
                        game.createmenu(Menu::credits25, true);
                    } else {
            ...
            switch (game.menu_.getCurOptIdx()) {
                case 0:
                    ...
                    if (game.current_credits_list_index >=
                        ...
                        // There are more patrons. Refresh the menu with the
                        // next ones
                        game.createmenu(Menu::credits4, true);
                    }
                ...
                case 1:
                    ...
                    if (game.current_credits_list_index < 0) {
                        ...
                        game.current_credits_list_index =
                            SDL_arraysize(Credits::superpatrons) - 1 -
                            (SDL_arraysize(Credits::superpatrons) - 1) % 9;
                        game.createmenu(Menu::credits3, true);
                    } else {
                        // There are more patrons. Refresh the menu with the
                        // next ones
        ...
        case Menu::credits5:
            switch (game.menu_.getCurOptIdx()) {
                case 0:
                    ...
                    if (game.current_credits_list_index >=
                        (int)SDL_arraysize(Credits::githubfriends)) {
                        // No more GitHub contributors. Move to the next credits
                        // section
                        game.current_credits_list_index = 0;
                        game.createmenu(Menu::credits6, true);
                    } else {
                        // There are more GitHub contributors. Refresh the menu
                        // with the next ones
                        game.createmenu(Menu::credits5, true);
                    }
                ...
                case 1:
                    ...
                    if (game.current_credits_list_index < 0) {
                        // No more GitHub contributors. Move to the previous
                        // credits section
                        game.current_credits_list_index =
                            SDL_arraysize(Credits::patrons) - 1 -
                            (SDL_arraysize(Credits::patrons) - 1) % 14;
                        game.createmenu(Menu::credits4, true);
                    } else {
                        // There are more GitHub contributors. Refresh the menu
                        // with the next ones
        ...
        case Menu::credits6:
            switch (game.menu_.getCurOptIdx()) {
                ...
                case 1:
                    ...
                    music.playef(11);
                    game.current_credits_list_index =
                        SDL_arraysize(Credits::githubfriends) - 1 -
                        (SDL_arraysize(Credits::githubfriends) - 1) % 9;
                    game.createmenu(Menu::credits5, true);
                    game.menu_.setCurOptIdx(1);
                    map.nexttowercolour();
                    break;
        ...
        case Menu::play: {
            ...
            if (game.menu_.getCurOptIdx() == 0) {
                // continue
                // right, this depends on what saves you've got
                if (!game.save_exists()) {
                    // You have no saves but have something unlocked, or you
                    // couldn't have gotten here
                    music.playef(11);
                    game.mainmenu = 0;
                    graphics.fademode = 2;
                } else if (game.telesummary == "") {
                    // You at least have a quicksave, or you couldn't have
                    // gotten here
                    music.playef(11);
                    game.mainmenu = 2;
                    graphics.fademode = 2;
                } else if (game.quicksummary == "") {
                    // You at least have a telesave, or you couldn't have gotten
                    // here
                    music.playef(11);
                    game.mainmenu = 1;
                    graphics.fademode = 2;
            ...
            } else if (game.menu_.getCurOptIdx() == 1 && game.unlock[8]) {
            ...
            } else if (game.menu_.getCurOptIdx() == sloffset + 2) {
                // play modes
                music.playef(11);
                game.createmenu(Menu::playmodes);
                map.nexttowercolour();
            } else if (game.menu_.getCurOptIdx() == sloffset + 3 &&
                       game.save_exists()) {
                // newgame
                music.playef(11);
                game.createmenu(Menu::newgamewarning);
                map.nexttowercolour();
            } else if (game.menu_.getCurOptIdx() == sloffset + ngoffset + 4) {
                // back
                music.playef(11);
                game.menu_.back();
                map.nexttowercolour();
        ...
        case Menu::cleardatamenu:
            ...
            break;
        case Menu::playmodes:
            if (game.menu_.getCurOptIdx() == 0 && game.slowdown == 30 &&
                !map.invincibility) // go to the time trial menu
            {
                music.playef(11);
                game.createmenu(Menu::timetrials);
                map.nexttowercolour();
            } else if (game.menu_.getCurOptIdx() == 1 && game.unlock[16]) {
                // intermission mode menu
                music.playef(11);
                game.createmenu(Menu::intermissionmenu);
                map.nexttowercolour();
            } else if (game.menu_.getCurOptIdx() == 2 && game.unlock[17] &&
                       game.slowdown == 30 &&
                       !map.invincibility) // start a game in no death mode
            {
                music.playef(11);
                game.createmenu(Menu::startnodeathmode);
                map.nexttowercolour();
            } else if (game.menu_.getCurOptIdx() == 3 &&
                       game.unlock[18]) // enable/disable flip mode
            {
            ...
            } else if (game.menu_.getCurOptIdx() == 4) {
                // back
                music.playef(11);
                game.menu_.back();
                map.nexttowercolour();
        ...
        case Menu::gameover2:
            ...
            music.playef(11);
            music.play(6);
            game.menu_.back(Menu::playmodes);
            map.nexttowercolour();
            break;
        ...
        case Menu::timetrials:
            if (game.menu_.getCurOptIdx() == 0 &&
                game.unlock[9]) // space station 1
            {
                music.playef(11);
                game.mainmenu = 3;
                graphics.fademode = 2;
            } else if (game.menu_.getCurOptIdx() == 1 && game.unlock[10]) // lab
            {
                music.playef(11);
                game.mainmenu = 4;
                graphics.fademode = 2;
            } else if (game.menu_.getCurOptIdx() == 2 &&
                       game.unlock[11]) // tower
            {
                music.playef(11);
                game.mainmenu = 5;
                graphics.fademode = 2;
            } else if (game.menu_.getCurOptIdx() == 3 &&
                       game.unlock[12]) // station 2
            {
                music.playef(11);
                game.mainmenu = 6;
                graphics.fademode = 2;
                       ...
                       game.unlock[13]) // warp
            {
                ...
                game.mainmenu = 7;
                graphics.fademode = 2;
            } else if (game.menu_.getCurOptIdx() == 5 &&
                       game.unlock[14]) // final
            {
                music.playef(11);
                game.mainmenu = 8;
                graphics.fademode = 2;
            } else if (game.menu_.getCurOptIdx() ==
                       6) // go to the time trial menu
            {
                // back
                music.playef(11);
                game.menu_.back();
        ...
        case Menu::timetrialcomplete3:
            switch (game.menu_.getCurOptIdx()) {
                case 0:
                    ...
                    music.playef(11);
                    music.play(6);
                    game.menu_.back(Menu::timetrials);
                    map.nexttowercolour();
                    break;
                case 1:
                    // Ok but first quickly remove the last stack frame to
                    // prevent piling up timetrialcomplete stack frames
                    game.menu_.back();
...
void titleinput()
{
    ...
    //|| key.isDown(KEYBOARD_UP) || key.isDown(KEYBOARD_DOWN)) game.press_action
    //= true; //on menus, up and down don't work as action
    if (key.isDown(KEYBOARD_ENTER))
        game.press_map = true;

    // In the menu system, all keypresses are single taps rather than holds.
    // Therefore this test has to be done for all presses
    ...
    if (!game.jumpheld && graphics.fademode == 0) {
        ...
        if (key.isDown(27) &&
            game.menu_.getCurOptName() != Menu::youwannaquit &&
            game.menustart) {
            music.playef(11);
            if (game.ingame_titlemode) {
                game.returntopausemenu();
        ...
        if (game.menu_.getCurOptName() == Menu::controller &&
            game.menu_.getCurOptIdx() > 0 && game.menu_.getCurOptIdx() < 5 &&
            key.controllerButtonDown()) {
            updatebuttonmappings(game.menu_.getCurOptIdx());
        }
    }
...
void gameinput()
{
    ...
    if (!script.running) {
        ...
        if (key.isDown(KEYBOARD_z) || key.isDown(KEYBOARD_SPACE) ||
            key.isDown(KEYBOARD_v) || key.isDown(KEYBOARD_UP) ||
            key.isDown(KEYBOARD_DOWN) || key.isDown(KEYBOARD_w) ||
        ...
        if (obj.entities[ie].rule == 0) {
            if (game.hascontrol && game.deathseq == -1 && game.lifeseq <= 5) {
                if (game.press_map && !game.mapheld) {
                    ...
                    if (game.activetele && game.readytotele > 20 &&
                        !game.intimetrial) {
                        if (int(std::abs(obj.entities[ie].vx)) <= 1 &&
                            int(obj.entities[ie].vy) == 0) {
                            ...
                            if (game.teleportscript != "") {
                                ...
                                // which teleporter script do we use? it depends
                                // on the companion!
                                game.state = 4000;
                                game.statedelay = 0;
                                ...
                                graphics.menuoffset =
                                    240; // actually this should count the
                                         // roomname
                ...
                if ((key.isDown(27) || key.isDown(game.controllerButton_esc)) &&
                    ...
                    BlitSurfaceStandard(
                        graphics.menubuffer, NULL, graphics.backBuffer, NULL);
                    graphics.menuoffset =
                        240; // actually this should count the roomname
                    graphics.oldmenuoffset = 240;
                    if (map.extrarow) {
                ...
                if ((key.isDown(SDLK_r) ||
                     key.isDown(game.controllerButton_restart)) &&
                    !game.nodeathmode) // && map.custommode) //Have fun
                                       // glitchrunners!
                {
                    game.deathseq = 30;
                }
                ...
                // Simple detection of keypresses outside player control, will
                // probably scrap this (expand on advance text function)
                if (!game.press_action) {
                    game.jumppressed = 0;
                    game.jumpheld = false;
                }
...
void mapinput()
{
    ...
    if (game.glitchrunnermode && graphics.fademode == 1 &&
        ...
        // glitchrunner mode.
        // Also have to check graphics.menuoffset so this doesn't run every
        // frame
    ...
    if (graphics.menuoffset == 0 &&
        ((!game.glitchrunnermode && game.fadetomenudelay <= 0 &&
          game.fadetolabdelay <= 0) ||
         graphics.fademode == 0)) {
        if (graphics.flipmode) {
            if (key.isDown(KEYBOARD_LEFT) || key.isDown(KEYBOARD_DOWN) ||
                key.isDown(KEYBOARD_a) || key.isDown(KEYBOARD_s) ||
        ...
        if (game.menupage < 12 ||
            (game.menupage >= 30 && game.menupage <= 33)) {
            if (key.isDown(KEYBOARD_ENTER) ||
                key.isDown(game.controllerButton_map))
                game.press_map = true;
            ...
            if (key.isDown(KEYBOARD_ENTER) || key.isDown(27) ||
                key.isDown(game.controllerButton_map))
                game.press_map = true;
        }
    ...
    if (!game.jumpheld) {
        ...
        if (game.menupage < 0)
            game.menupage = 3;
        if (game.menupage > 3 && game.menupage < 9)
            game.menupage = 0;
    ...
    switch (game.menupage) {
        case 1:
            if (obj.flags[67] && !game.inspecial() && !map.custommode) {
                ...
                // which teleporter script do we use? it depends on the
                // companion!
                game.state = 4000;
                game.statedelay = 0;
            }
        ...
        case 11:
            ...
            // Kill contents of offset render buffer, since we do that for some
            // reason. This fixes an apparent frame flicker.
        ...
        case 33:
            ...
            map.bg_to_kludge();
            game.menu_.kludge_ingametemp = game.menu_.getCurOptName();
        ...
        if (game.press_left && any_tele_unlocked) {
            do {
                ...
                if (game.teleport_to_teleporter < 0)
                    game.teleport_to_teleporter = map.teleporters.size() - 1;
            ...
            do {
                game.teleport_to_teleporter++;
                if (game.teleport_to_teleporter >= (int)map.teleporters.size())
