    ...
    while (SDL_PollEvent(&evt)) {
        switch (evt.type) {
            ...
            case SDL_KEYDOWN: {
                ...
                if (textentry()) {
                    if (evt.key.keysym.sym == SDLK_BACKSPACE &&
                        ...
                        keybuffer =
                            keybuffer.substr(0, iter - keybuffer.begin());
                        if (keybuffer.empty()) {
            ...
            case SDL_CONTROLLERDEVICEADDED: {
                SDL_GameController* toOpen =
                    SDL_GameControllerOpen(evt.cdevice.which);
                printf("Opened SDL_GameController ID #%i, %s\n",
                       evt.cdevice.which,
                       SDL_GameControllerName(toOpen));
                controllers[SDL_JoystickInstanceID(
                    SDL_GameControllerGetJoystick(toOpen))] = toOpen;
                break;
            }
                ...
                switch (evt.window.event) {
                    ...
                    case SDL_WINDOWEVENT_FOCUS_GAINED:
                        ...
                        if (!game.disablepause && Mix_PlayingMusic()) {
                            ...
                            music.songStart +=
                                SDL_GetPerformanceCounter() - pauseStart;
                        }
                        break;
                    case SDL_WINDOWEVENT_FOCUS_LOST:
                        ...
                        if (!useFullscreenSpaces) {
                            ...
                            SDL_SetWindowFullscreen(
                                SDL_GetWindowFromID(evt.window.windowID), 0);
                        }
...
bool KeyPoll::controllerWantsLeft(bool includeVert)
{
    return (buttonmap[SDL_CONTROLLER_BUTTON_DPAD_LEFT] || xVel < 0 ||
            (includeVert &&
             (buttonmap[SDL_CONTROLLER_BUTTON_DPAD_UP] || yVel < 0)));
}
...
bool KeyPoll::controllerWantsRight(bool includeVert)
{
    return (buttonmap[SDL_CONTROLLER_BUTTON_DPAD_RIGHT] || xVel > 0 ||
            (includeVert &&
             (buttonmap[SDL_CONTROLLER_BUTTON_DPAD_DOWN] || yVel > 0)));
}
